...
public interface QuestionMapper {
    ...
    @Select("select *from question  order by gmt_create desc limit #{pageStart},#{pageSize} ")
    List<Question> selectAll(Integer pageStart,Integer pageSize);
    @Select("select count(1) from question")
    Integer selectCount();
    @Select("select count(1) from question where creator=#{creator}")
    int selectCountByUserID(Integer creator);
    ...
    @Update("update question set title=#{title},description=#{description},tag=#{tag},gmt_modified=#{gmtModified} " +
            "where id=#{id}")
    int updateById(Question dbQuestion);
}
